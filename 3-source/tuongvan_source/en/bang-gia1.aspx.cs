    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["bg"]))
            {
                ...
                var dv = oProduct.ProductSelectOne(Request.QueryString["bg"]).DefaultView;

                if (dv != null && dv.Count <= 0) return;
                var row = dv[0];

                strTitle = "Price list " + Server.HtmlDecode(row["ProductNameEn"].ToString());
            ...
            {
                strTitle = strMetaTitle = "Price List";
                strDescription = "";
                strMetaDescription = "";
            }
